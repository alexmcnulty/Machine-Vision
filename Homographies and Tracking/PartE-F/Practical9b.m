...
%particles_old(:,3) = randn(numParticles,1);
%particles_old(:,4) = randn(numParticles,1);
% ============================
...
for( iTime = 1:22 )
    ...
    for (particleNum = 1:numParticles)
        ...
        % motion model.
        noisePos = 10*randn(1,2);
        noiseVel = rand(1,2);
        particles_new(particleNum,1:2) = particles_old( samples_to_propagate(particleNum),1:2 ) +noisePos(1,:);
        ...
        %particles_new(particleNum,3:4) = particles_old( samples_to_propagate(particleNum),3:4 ) + noiseVel(1,:);
        %particles_new(particleNum,3:4) = round(  particles_new(particleNum,3:4)  );
    end;
    ...
    [sortedX,sortingIndices] = sort(weight_of_samples,'descend');

    % sortedX will be in descending order. Therefore, the first N elements will be the N maximum values.

    maxValues = sortedX(1:10);
    maxValueIndices = sortingIndices(1:10);
    high1 = particles_new(maxValueIndices,2);
    high2 = particles_new(maxValueIndices,1);
    ...
    % Optional code to save out figure:
        % pngFileName = sprintf( '%s_%.5d.png', 'myOutputb', iTime );
