function r=HW2_Practical9cOrig( templateMetaFileName )
%Make sure to run when in pattern01 folder
%Need to take some image samples 1's 40's 70's 80's and 100's again, take 4
%I'd say.
close all;
%
...
for( iTime = 1:numFrames )
    ...
    for particleNum = 1:numParticles
        ...
        % motion model.
        noise = 14*randn(1,2);
        particles_new(particleNum,:) = particles_old( samples_to_propagate(particleNum),: ) + noise(1,:);
    ...
    weightedAve = [weight_of_samples'*particles_new(:,1) weight_of_samples'*particles_new(:,2)];
    ...
    % Optional code to save out figure:
     %pngFileName = sprintf( '%s_%.5d.png', 'myOutputll', iTime );
