...
%load in training data - contains two variables each of size 3 x 10,000
...
covRG = (R - meanData(1))*(G - meanData(2))'/(N-1);
covRB = ((R - meanData(1))*(B - meanData(3))')/(N-1);
covGB = ((G - meanData(2))*(B - meanData(3))')/(N-1);
covGR = covRG;
covBR = covRB;
covBG = covGB;
covData = [var(R) covRG covRB; covGR var(G) covGB; covBR covBG var(B)];
matlab =cov(data');
