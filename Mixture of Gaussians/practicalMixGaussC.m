...
drawnow;
%saveas( gcf, 'PartC_iteration_initial', 'jpg' );
...
for (cData =1:nData)
    ...
    data(:,cData) = mixGauss.mean(:,h)+chol(mixGauss.cov(:,:,h))'*randn(2,1);
...
responsibilities_numerator = zeros(k,nData);
nIter = 20;
for (cIter = 1:nIter)
   %Expectation step
   ...
   for (cData = 1:nData)
        ...
        %replace this:
        for (row=1:k)
        responsibilities_numerator(row,cData)=calcGaussianProb(data(:,cData),mixGaussEst.mean(:,row),mixGaussEst.cov(:,:,row))*mixGaussEst.weight(:,row);
        end;
        
   end;
   for(column=1:nData)
     postHidden(:,column)=responsibilities_numerator(:,column)./sum(responsibilities_numerator(:,column));
   end;
   ...
   for (cGauss = 1:k) 
        ...
        mixGaussEst.weight(cGauss) = sum(postHidden(cGauss,:))/length(postHidden(cGauss,:)); 
        ...
        mixGaussEst.cov(:,:,cGauss) = bsxfun(@times,p1, postHidden(cGauss,:))*p1'./sum(postHidden(cGauss,:));
                  mixGaussEst.cov(:,:,cGauss)=((mixGaussEst.cov(:,:,cGauss)+mixGaussEst.cov(:,:,cGauss)')/2);
       %(Came up for part 2 but put back in here to speed up process.) 
   end;
   
   %draw the new solution
   drawEMData2d(data,mixGaussEst);drawnow;
   filename = sprintf('PartC_iteration_%d.jpg', cIter);
   %saveas(gcf, filename, 'jpg');
   fprintf('Log Likelihood Iter %d: %4.3f\n',cIter,logLike)
   %saveas( gcf, 'PartC_iteration_20', 'jpg' );
   %calculate the log likelihood   logLike =
   %getMixGaussLogLike(data,mixGaussEst);   fprintf('Log Likelihood Iter %d
   %: %4.3f\n',cIter,logLike);
end;
...
for(cData = 1:nData)
    ...
    for (num= 1:size(mixGaussEst.weight))
    like_n=calcGaussianProb(thisData,mixGaussEst.mean(:,num),mixGaussEst.cov(:,:,num)).* mixGaussEst.weight(:,num) ;  
    like = like + like_n;
    end;
...
prob = (1/((2*pi)*sqrt(det(var))))*exp(-0.5*(x-mean)'*(var)^-1*(x-mean));
...
% we need to run the function several times to find the best starting theta
% see page 114 in the book. 
